...
def choose_prime(n):
    if n == 32 * 32 * 3:
        ...
        assert False, 'Please specify the prime or the power of a prime given {}'.format(
            n)
...
class RootFind(Function):
    ...
    def banach_find_root(nnet_z, nnet_x, z0, x, *args):
        eps = args[-2]
        # Can also set this to be different, based on training/inference
        threshold = args[-1]
        x_embed = nnet_x(x) + x
        def g(z): return x_embed - nnet_z(z)
    ...
    def broyden_find_root(nnet_z, nnet_x, z0, x, *args):
        ...
        threshold = args[-1]
        x_embed = nnet_x(x) + x
        def g(z): return x_embed - nnet_z(z) - z
        result_info = broyden(g, torch.zeros_like(z0).to(
            z0), threshold=threshold, eps=eps, name="forward")
...
class imBlock(nn.Module):
    ...
    class Backward(Function):
        ...
        def backward(ctx, grad):
            ...
            def g(x_):
                ...
                Fz.backward(x_, retain_graph=True)
            ...
            result_info = broyden(
                g, dl_dh, threshold=threshold, eps=eps, name="backward")
    ...
    def forward(self, x, logpx=None, restore=False):
        ...
        self.nnet_x_copy.load_state_dict(
            self.nnet_x.state_dict().to(device='cpu'))
        self.nnet_z_copy.load_state_dict(
            self.nnet_z.state_dict().to(device='cpu'))
    ...
    def _logdetgrad(self, z, x):
        ...
        with torch.enable_grad():
            ...
            if self.training:
                ...
                def coeff_fn(k): return 1 / rcdf_fn(k, self.n_exact_terms_test) * \
            ...
            if not self.exact_trace:
                ...
                vareps_x = torch.distributions.bernoulli.Bernoulli(
                    torch.Tensor([0.5])).sample(x.shape).reshape(x.shape).to(x) * 2 - 1
                vareps_z = torch.distributions.bernoulli.Bernoulli(
                    torch.Tensor([0.5])).sample(z.shape).reshape(z.shape).to(z) * 2 - 1
                ...
                if self.training and self.grad_in_forward:
                    ...
                    Fx = self.nnet_x(x)
                    Fz = self.nnet_z(z)
                    logdet_x = estimator_fn(
                        Fx, x, n_power_series, vareps_x, coeff_fn, self.training)
                    logdet_z = estimator_fn(
                        Fz, z, n_power_series, vareps_z, coeff_fn, self.training)
                    logdetgrad = logdet_x - logdet_z
            else:
                ############################################
                ...
                for k in range(2, n_power_series + 1):
                    ...
                    logdetJx = logdetJx + (-1)**(k+1) / \
                        k * coeff_fn(k) * batch_trace(Jx_k)
                Fz = self.nnet_z(z)
                Jz = batch_jacobian(Fz, z)
                ...
                for k in range(2, n_power_series + 1):
                    ...
                    logdetJz = logdetJz + (-1)**(k+1) / \
                        k * coeff_fn(k) * batch_trace(Jz_k)
...
def batch_jacobian(g, x, create_graph=True):
    ...
    for d in range(g.shape[1]):
        jac.append(torch.autograd.grad(torch.sum(g[:, d]), x, create_graph=create_graph)[
